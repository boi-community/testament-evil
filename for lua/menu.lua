local dssmod = RegisterMod("Dead Sea Scrolls (Testament Evil)", 1)
local game = Game()
local sfx = SFXManager()

local dssmenu = DeadSeaScrollsMenu
local dummytempvalue = nil
local mfdat = {}
...
if not dssmenu or (dssmenu.CoreVersion < dssCoreIncluded) then
    ...
    dssmenu.CoreMod = "Testament Evil"
    DeadSeaScrollsMenu = dssmenu
end

function MenuProvider.IsMenuCore()
    return DeadSeaScrollsMenu.CoreMod == "Testament Evil"
end
...
local helps = {
    {"controls", fsize = 3, tooltip = {'use m+kb!', 'gamepad', 'unsupported!'}},
    {{'skills', ''}, nosel = true},
    {{'weapon skill', 'left shift'}, fsize = 1, nosel = true},
    {{'knife skill', 'double rclick + hold'}, fsize = 1, nosel = true},
    '',
    {{'grenades', ''}, nosel = true},
    {{'throw', 'left click'}, fsize = 1, nosel = true},
    {{'lob', 'right click'}, fsize = 1, nosel = true},
    '',
    {"crits", fsize = 3, nosel = true},
    "some guns can be charged",
    {"by holding fire!", nosel = true},
    {"releasing at the right", nosel = true},
    {"time shoots a crit,", nosel = true},
    {"dealing more damage", nosel = true},
    {"and stunning enemies!", nosel = true},
    '',
    "knifing a stunned",
    {"enemy deals", nosel = true},
    {"critical knife damage!", nosel = true},
    '',
    "critical hits are",
    {"a great way to save", nosel = true},
    {"on ammo.", nosel = true},
    '', 
    {"skills", fsize = 3, nosel = true},
    {"the two bars at the", tooltip = {'left bar:', 'weapon skill', 'right bar:', 'knife skill'}},
    {"top of the screen", nosel = true},
    {"are skill bars.", nosel = true},
    {"different weapons have", nosel = true},
    {"different skills!", nosel = true},
    '',
    {"items", fsize = 3, nosel = true},
    "some items are passive and",
    {"work like usual.", nosel = true},
    {"most are weapon mods!", nosel = true},
    {"picking up weapon mods", nosel = true},
    {"will store them in the", nosel = true},
    {"attache case for later.", nosel = true},
    '',
    {"merchant", fsize = 3, nosel = true},
    "the merchant appears",
    {"at the start of every", nosel = true},
    {"floor past basement.", nosel = true},
    '',
    "pay him coins to",
    {"purchase new guns,", nosel = true},
    {"or to attach a mod", nosel = true},
    {"to a gun.", nosel = true},
    '',
    "guns have perks",
    {"when upgraded!", nosel = true},
    {"most get a larger", nosel = true},
    {"clip, but some", nosel = true},
    {"have other modifiers.", nosel = true},
    '',
    "the timer at the top",
    {"is a par timer!", nosel = true},
    '',
    "beat the floor before",
    {"the par timer to", nosel = true},
    {"get an early bird", nosel = true},
    {"discout from the", nosel = true},
    {"merchant.", nosel = true},
    '',
    {"herbs", fsize = 3, nosel = true},
    "herbs can be used to",
    {"heal, increase max hp,", nosel = true},
    {"or gain soul hearts!", nosel = true},
    '',
    "the following is a",
    {"list of herbs:", nosel = true},
    {"green", fsize = 1, tooltip = "1/2 heart"},
    {"green x2", fsize = 1, tooltip = "1/12 hearts"},
    {"green x3", fsize = 1, tooltip = "full heal"},
    {"green + red", fsize = 1, tooltip = "full heal"},
    {"green + blue", fsize = 1, tooltip = {"1/2 soul", "heart"}},
    {"green + blue + blue", fsize = 1, tooltip = "1 soul heart"},
    {"green + yellow", fsize = 1, tooltip = {"hp up", "1/2 heart"}},
    {"green + red + yellow", fsize = 1, tooltip = {"hp up", "full heal"}},
    {"green + blue + yellow", fsize = 1, tooltip = {"hp up", "1 soul heart"}},
    '',
    {"notes", fsize = 3, nosel = true},
    "disable found hud",
    {"and extra hud,", nosel = true},
    {"testament evil takes", nosel = true},
    {"care of them", nosel = true},
    '',
    "don't use hard mode,",
    {"testament evil has", nosel = true},
    {"its own difficulty", nosel = true},
    {"settings if you", nosel = true},
    {"need them", nosel = true},
}

local function integrateIntoOption(option, entries)
    for _, entry in ipairs(entries) do
        if type(entry) == "string" then
            leondirectory[option].buttons[#leondirectory[option].buttons + 1] = {str = entry, nosel = entry == ''}
        else
            local button = {}
    
            if type(entry[1]) == "table" then button.strpair = {{str = entry[1][1]}, {str = entry[1][2]}} else button = {str = entry[1]} end
            if entry.fsize then button.fsize = entry.fsize end
            if entry.tooltip then
                if type(entry.tooltip) == "string" then
                    entry.tooltip = {entry.tooltip}
                end
                button.tooltip = {strset = entry.tooltip}
            end
            if entry.nosel then button.nosel = entry.nosel end
            leondirectory[option].buttons[#leondirectory[option].buttons + 1] = button
        end
    end 
end

integrateIntoOption("help", helps)
integrateIntoOption("credits", credits)

local leondirectorykey = {
    Item = leondirectory.main,
    Main = 'main',
    Idle = false,
    MaskAlpha = 1,
    Settings = {},
