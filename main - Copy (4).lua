...
mfdat["'"] = {36,1,4,5};   mfdat['"'] = {37,3,4,5};   mfdat[':'] = {38,1,3,4};
...
mfdat['%'] = {52,4,6,8}; 	 mfdat['_'] = {53,2,4,5};		mfdat[' '] = {55,4,6,8};
mfdat['='] = {54,5,8,12};
	...
	if menuactive then
		...
		if draw then
			...
			if item.title then
				...
				local title = {type = 'str', index = item.title, size = 3, root = root1, pos = Vector(0, -72), halign = 0, underline = true}
			...
			if item.buttons then
				local soffset = 0
				local doffset = 0
				local butind = #item.buttons
				local clamp = butind
				local dispmax = item.dispmax or 7
				local bheight = item.bheight or 16

				if butind > dispmax then
					clamp = (dispmax - 1) / 2
					doffset = math.min(math.max(bsel, clamp + 1), butind - clamp)
					soffset = (butind / 2)
				end

				log(item.scroll)
				--set scroll after adding all strings, base it off of combined heights
				--account for valigns or something I don't know
				--base shit purely off of if it can fit in the space or not

				if item.scroll then
					item.scroll = Lerp(item.scroll, soffset, .2)
				else
					item.scroll = soffset
				end
				local strset = {type = 'strset', set = {}, index = item, valign = item.valign or 0, root = root1, pos = Vector(0, 8 + ((item.scroll + butoff) * bheight))}
				local strheight = 0
				for i, btn in ipairs(item.buttons) do
					local clampalpha = 1
					if dispmax < butind then
						clampalpha = (clamp + 1) - math.abs((item.scroll + i) - (butind / 2))
						clampalpha = math.max(math.min(clampalpha, 1), 0)
					end

					local bdraw = {type = 'str', index = btn, size = 3, height = 18, alpha = clampalpha, select = bsel == i}
					table.insert(strset.set, bdraw)

					if btn.variable then
						local vdraw = {type = 'str', str = btn.choices[btn.setting], setind = btn, size = 2, height = 16, alpha = clampalpha, pos = Vector(0, -4)}
						table.insert(strset.set, vdraw)
					end
				end
	--			strset.pos = Vector(0, 4 + (strheight))
...
function l.drawMenu(tab)
	...
	elseif dtype == 'str' then
		...
		if update or not ind.init then
		...
		font.Color = color
		font.Scale = scale
		local xoff = (ind.halign == 0 and ind.len / -2) or (ind.halign == 1 and ind.len * -1) or 0
		local yoff = ((ind.valign == 0 and -.5) or (ind.valign == 1 and -1) or 0) * (myscale)
		for i, chr in ipairs(ind.chr) do
			local fpos = pos + Vector(chr[2], 0) + Vector(xoff, yoff)
			--[[local clipt = 0
			local clipb = 0
			if tab.box1 then
				clipt = math.min(math.max(0, tab.box1.Y - fpos.Y), myscale)
			end
			if tab.box2 then
				clipb = math.min(math.max(0, fpos.Y - tab.box2.Y), myscale)
			end
			if clipt < myscale and clipb < myscale then
			...
			if alpha > 0 then
				font:SetFrame(fname, mfdat[chr[1] ][1])
				font:Render(root + fpos, Vector(0, 0), Vector(0, 0))
			end

			--font:RenderLayer(layer, root + pos + Vector(chr[2], 0) + Vector(xoff, yoff))
		end
		--underline
		...
		if tab.select then
			...
			menuspr.Color = color
			menuspr:RenderLayer(3, root + pos + Vector(xoff - 12, -1))
		end
		--settings
		local setind = tab.setind
		if setind and setind.choices and #setind.choices > 1 then
			menuspr.Color = color
			if setind.setting > 1 then
				menuspr:SetFrame("Sym", 8)
				menuspr:RenderLayer(3, root + pos + Vector(-60, -1))
			end
			if setind.setting < #setind.choices then
	...
	elseif dtype == 'strset' then
		...
		local yo = (((tab.valign or 0) + 1) * yy) / -2
