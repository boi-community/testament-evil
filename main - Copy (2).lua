...
function l:post_update()
	if gVAR.gunmode then
		...
		for i, gun in ipairs(gVAR.guns) do
			...
			--if gun.fx.missenemy then
			--	log(gun.fx.missenemy[1][1].cancel[1].undo)
			--end
	...
	if gVAR.gunmode then
		...
		if d.grenade then
			...
			elseif not d.landed then
				...
				d.fallspeed = d.fallspeed + (d.fallaccel or 1.2)
									...
									for j, addtrig in ipairs(value) do
										...
										if trigtab.undo then
											for i, ncond in ipairs(trigtab.undo) do
												gun.trig[ncond] = gun.trig[ncond] or {}
												table.insert(gun.trig[ncond], {link = newtrig, reset = trigtab.reset})
											end
										end
									end
								end
...
function l.gunMove(en, gun)
	...
	if gun and not game:IsPaused() then
		for h, fam in ipairs(d.gunners) do
			...
			local disttgt = (gun.gunorbit and 20 or 0) + mysize + 6 + gWEP.BaseLength[gun.base] + (gun.holddist or 0)
			...
			if weapons then
				for i = 1, 4 do
					...
					for j, wep in ipairs(set) do
						...
						local offset = Vector(distoff, 0):Rotated(gun.gunorbit and (offang + (game:GetFrameCount()*2) % 360) or myrot)
						...
						if not (gun.stickwhenfiring and gc.aiming and fam.Type ~= 3) then
							...
							if gun.stickwhenfiring and fam.Type == 1 then
								if gun.kingbaby and gun.kingbaby:Exists() then
									...
									wep.Velocity = (topos - wep.Position) * .98
									wep.PositionOffset = Vector(0, anim.height)
									--gun.kingbaby.Position = d.aimfrompoint
									--gun.kingbaby.Velocity = Vector(0, 0)
								end
							end
						end
...
function l.gunUse(en, gun)
	if gun then
			...
			if not gun.reloading then
							...
							if gc.shoot or gun.forceshot then
								...
								--to shoot
									...
									if gun.sp[ESP.brimstone] then
										--gun.firingshots = math.ceil((gun.chargebonus * gun.chamber * gun.damagemult * stat.Damage) / (stat.Damage * 4))
										--gun.firingshots = math.ceil(((gun.chamber / gun.ammoshots) * ((stat.Damage * gun.damagemult) / .666) / 2))
									end
				...
				if gun.firingshots > 0 then
					...
					if gun.freeshots == 0 then
						if (rng:RandomFloat() * (1 + (gun.freeshotchance or 0))) > 1 then
							gun.freeshots = 1
						end
						gun.firingshots = gun.firingshots - (1 / gun.shotsmult)
					else
						gun.freeshots = 0
					end
					gun.using = true
		...
		if player then
			...
			if isfirst or issecond then
				local max1 = config:GetCollectible(active) and config:GetCollectible(active).MaxCharges
						...
						for k = 1, pelletnum do
							...
							--if (not gun.alternatinggunfire) or (gun.fireevents % (gun.alternatinggunfire or 1) == (j % (gun.alternatinggunfire or 1))) then
								...
								if en.Type == 1 then
									...
									if grenade then
										shot = Isaac.Spawn(4, gCON.Variant.Grenade, 0, wd.tearspawn, vel, wd.wielder)
										shot.PositionOffset = wep.PositionOffset
										sd = shot:GetData()
										sd.grenade = 1
										sd.blowonhit = true
										shot.EntityCollisionClass = EntityCollisionClass.ENTCOLL_ENEMIES
										shot:ToBomb():GetData().fallspeed = -1; sd.fallspeed = -1
										sd.fallaccel = .5
										shot:GetSprite():ReplaceSpritesheet(0, pocket['gr'].sprite2)
										shot:GetSprite():LoadGraphics()
										shot:GetSprite():Play("Idle")
									elseif flame then
								...
								elseif grenade then
									...
									log((d.gunstat.TearHeight * rangebonus))
								elseif flame then
...
function l:tear_collision(tear, ent, low)
	if gVAR.gunmode then
		...
		if sd.isshot then
			...
			if gun and gun.trig.hitenemy and ent:IsEnemy() and not sd.hitenemy then
