...
pcall(require, "for lua/usersettings"); l.data.set = gunmod_SET; local gSET = gunmod_SET
	screensize = gSET.ScreenSize or screensize
--balance values
...
mfdat['g'] = {6,5,8,12};   mfdat['h'] = {7,4,8,13};   mfdat['i'] = {8,1,3,4};
...
mfdat['-'] = {49,4,6,8};   mfdat['X'] = {50,5,6,8};   mfdat['D'] = {51,5,6,8};
...
local mmenudir = {
	--LEVEL 1
	main = {
		title = 'testament evil',
		radial = true, -- todo: implement if I care enough
		buttons = {
			{str = 'resume game', action = 'resume'},
			{str = 'inventory', dest = 'inventory'},
			{str = 'game settings', dest = 'game_settings'},
			{str = 'other settings', dest = 'other_settings'},
			{str = 'help me!!!', dest = 'help'},
			--[[{str = 'what the'},
			{str = 'fuck is your'},
			{str = 'problem you'},
			{str = 'dumb son of'},
	...
	other_settings = {
		...
		dispmax = 3, bheight = 32,
		buttons = {
			{str = 'hud opacity',
				increment = 25, max = 100, suf = "%",
				variable = 'HudOpacity',
				setting = 50,
			},
			{str = 'show stats',
			...
			{str = 'bar style',
				choices = {'thick', 'thin'},
				tip = {"if you look very closely you might see what this does"},
				variable = 'BarThin',
				setting = 1,
			},
			{str = 'gib limit',
			...
			{str = 'light saturation',
				increment = 25, max = 200, suf = "%",
				variable = 'LightingSaturation',
				setting = 100,
			},
			{str = 'shot brightness',
				increment = 10, max = 100, suf = "%",
				variable = 'ShotFlashBrightness',
				setting = 50,
			},
			{str = 'menu palette',
				choices = {'classic', 'soy milk', 'phd', 'faded polaroid', 'missing page 2',
					'???', 'succubus', 'birthright', 'impish', 'ice tray', 'queasy', 'fruitcake',
					'delirious'},
...
function hsvToRgb(h, s, v, a) --credit EmmanuelOga
  local r, g, b
...
function l:leonCmd(cmd, params)
	...
	elseif cmd == 'reroom' then
		if params == 'l' or params == 'loop' then
			if gVAR.looproom then
				gVAR.looproom = false
			else
				gVAR.looproom = true
			end
		else
			player:UseActiveItem(437)
		end
...
function l.getInput(pnum)
	...
	input.mode = 0;
	if (not mmenuvar.open) and CSP.MenuFace:IsFinished("Dissapear") then
		--welcome to hell
		input.mode = 1
...
function l:post_update()
	if gVAR.gunmode then
		...
		for i, en in ipairs(Isaac.GetRoomEntities()) do
			...
			--ENMIESS
			if en:IsEnemy() and en.MaxHitPoints > 0 then
				--LAST ENEMY SPOTTED
				if en:ToNPC().CanShutDoors then
				...
				--gun baby target acquirement
				for j, fam in ipairs(gVAR.gunusers) do
					if fam.Type == 3 and en:IsVulnerableEnemy() then
						local fd = fam:GetData()
						if fd.famgunai and fd.mygun then
							local fgun = fd.mygun
							local diff = en.Position - fam.Position
							local dist = diff:Length()
							local frame = game:GetFrameCount()
							if (not fd.target and fd.targetdist) or (frame % 10 == 0) then
								local hitpoint = en.Position - (diff:Normalized() * en.Size)
								local sight = fgun.famignorecol or room:CheckLine(fam.Position, hitpoint, 3, 9999, false, false)
								if (not fd.target) or (sight and ((not fd.targetinsight) or (dist < fd.targetdist))) then
									fd.target = en
									fd.targetdist = dist
								end
							end
			...
			--Custom gun babies
			if type == 3 and variant == gCON.Variant.LeonFam then
				for j, fam in ipairs(gVAR.gunusers) do
					if fam.Type == 3 and fam.Variant == gCON.Variant.LeonFam then
						local diff = en.Position - fam.Position
						local dist = diff:Length()
						if dist < 30 then
							en.Velocity = en.Velocity + ((en.Position - fam.Position):Normalized() * .35)
						end
					end
				end
			end
		...
		if gVAR.tempsee > 0 then
			gVAR.tempsee = gVAR.tempsee - 1
			if gVAR.tempsee == 0 then
				player:RemoveCollectible(76)
			end
		end
		if not game:IsPaused() then
...
function l:familiar_update(fam)
	if gVAR.gunmode then
		...
		if gWEP.FamWeps[tostring(variant)] or fam:GetData().famgunai then -- dem gun babies
			local player = Isaac.GetPlayer(0)
			local d = player:GetData()
			if not fd.gunuser then
				local newgun = {}
				l.gunUserInit(fam)
				l.gunInit(gWEP.FamWeps[tostring(variant)], true, newgun)
				l.gunSwitch(fam, newgun)
				fd.famgunai = true
				if not newgun.famnoposition then
					--fam.Variant = gCON.Variant.LeonFam
				end
			end

			if player:HasCollectible(247) and not fd.bffupgraded then--bff
				l.gunMod(fd.mygun, fd.mygun.upgrade[1])
				l.gunSwitch(fam, fd.mygun)
				fd.bffupgraded = true
			end

			--[[fam.Target = nil
			fam.TargetPosition = Vector(-500, -500)
			fam.FireCooldown = 100
			fam:PickEnemyTarget(0, 1000)--]]

			local gc = fd.guncontrol
			local fgun = fd.mygun
			local tgt = fd.target
			local custom = fam.Variant == gCON.Variant.LeonFam
			fd.targetinrange = false
			fd.targetinsight = false
			if tgt then
				if (not tgt:Exists()) then
					tgt = false
				elseif tgt:IsDead() then
					tgt = false
				else
					fd.targetinrange = fd.targetdist < fgun.famrangemax and fd.targetdist > fgun.famrangemin
					fd.targetinsight = fgun.famignorecol or game:GetRoom():CheckLine(fam.Position, fd.target.Position, 3, 9999, false, false)
				end
			end
			fd.target = tgt

			--fam:PickEnemyTarget(fgun.famrangemax or 200, 8)
			if tgt then
				local aimat = Vector(0, 0)
				local diff = tgt.Position - fam.Position
				fd.targetdist = diff:Length()
				aimat = tgt.Position + (tgt.Velocity * (diff:Length() / (fgun.velocity * 10)))
				aimat = game:GetRoom():GetClampedPosition(aimat, 20)
				gc.aim = (aimat - fam.Position):Normalized()
				gc.dotaim = aimat

				if not fgun.famnoposition then
					local findrange = (fgun.famrangepref or 80) + tgt.Size
					local diff2 = (tgt.Position - (diff:Normalized() * findrange)) - fam.Position
					fam.Velocity = fam.Velocity + (diff2:Normalized() * (custom and 1 or 2))
				end
			else
				if fgun.famgotoaim and d.input.mouse and not fgun.famnoposition then
					local diff = game:GetRoom():GetClampedPosition(d.input.aimpoint, 20) - fam.Position
					if math.abs(diff:Length() - 120) > 20 then
						local diff2 = (d.input.aimpoint - (diff:Normalized() * 120)) - fam.Position
						fam.Velocity = fam.Velocity + (diff2:Normalized() * (custom and 1 or 2))
					end
				end
			...
			if tgt then
				local aimed = math.abs(angleDiff(gc.aim:GetAngleDegrees(), fd.gunanim.dir)) < 5
				local valid = aimed and fd.targetinrange and fd.targetinsight and (fgun.firedelay <= 0) and (fgun.chamber > 0)
				gc.aiming = true
				gc.load = true
				if fgun.firemode == EFM.charge then
					--gc.load = true
					gc.charge = true
					gc.shoot = valid and fgun.charged and (rng:RandomFloat() < (fgun.famcritskill / fgun.critframes) or not fgun.crit)
					if not gc.shoot then
						fgun.clip = fgun.clipsize
					end
				else
					--gc.load = valid
					gc.shoot = valid
				...
				gc.load = false; gc.charge = false; gc.shoot = false; gc.aiming = false
			end
...
function l:pre_entity_spawn(type, variant, subtype, pos, vel, spawner, seed)
	...
	if gVAR.gunmode then
		...
		if spawner and spawner.Type == 6 then
			--return {1000, EffectVariant.EFFECT_NULL, 0}
		--elseif type == 2 and spawner.Type == 3 then
		--	if spawner.Variant == 2 and subtype == 0 then
		--		return {1000, EffectVariant.EFFECT_NULL, 0}
		--	end
		elseif type == 4 then
		...
		elseif type == 5 then
			if (pos.X % 40 == 0 and pos.Y % 40 == 0) then
				local grid = room:GetGridEntityFromPos(pos)
				if grid and grid.Desc.Type == 4 then
					--log(grid:ToRock().FrameCnt)
					--log(grid.Desc.Type..' '..grid.Desc.Variant..' '..grid.State)
					--todo: ???
				end
			end
			...
			elseif variant == 10 then
				...
				if subtype <= 5 then
					...
					if (pos.X % 40 == 0 and pos.Y % 40 == 0) then
						local grid = room:GetGridEntityFromPos(pos)
						if grid and grid.Desc.Type == 4 and grid:ToRock().FrameCnt == 0 then
							local newpick = randWeight(drop, drop.tintrockdrop)
							return {newpick[1], newpick[2], newpick[3], seed}
						end
					end
					local newpick = randWeight(drop, drop.heartdrop)
		...
		elseif type == 7 and variant == 2 and spawner.Type == 3 and spawner.Variant == 53 then
			return {1000, EffectVariant.EFFECT_NULL, 0}
...
function l:post_pickup_update(pickup)
	if gVAR.gunmode then
		...
		--we all fade away
		if gSET.Difficulty >= gBAL.temppickupsmindif and variant ~= 100 then
...
function l.getFlashlightColor()
	...
	local lsat = gSET.LightingSaturation / 100
		...
		if d.grenade then
			...
			if d.grenade == 3 then
				if land then
					...
					--if not player:HasCollectible(76) then
					--	player:AddCollectible(76, 0, false)
						--player:RemoveCostume(config:GetCollectible(76))
					--	player:RemoveCollectible(76)
						--gVAR.tempsee = 1
					--end
					...
					for i = 0, room:GetGridSize() - 1 do
						...
						if grid and grid.Desc.Type == GridEntityType.GRID_TNT then
							grid:Hurt(10)
						end
					end
				end
				...
				for i, en in ipairs(Isaac.GetRoomEntities()) do
					...
					if isenemy then
						...
						elseif d.grenade == 3 then
							if dist < 120 * rangemod then
								range = true
								if (en.MaxHitPoints < 7) or type == 231 or type == 304 or type == 292 then
									en:TakeDamage(en.MaxHitPoints * 2, 0, useref, 0)
								else
									en:TakeDamage(d.damage / 2, 0, useref, 0)
								end
							end
							confuse = confuse + ((mega and 12) or (mini and 3) or 8)
						end
						if range then
							if gBAL.explosiveenemies[type] == -1 or gBAL.explosiveenemies[type] == en.Variant then
								en:TakeDamage(en.MaxHitPoints * 2, 0, useref, DamageFlag.DAMAGE_EXPLOSION)
								if type == 292 then
									en:Kill()
								end
							end
						...
						if confuse > 0 and (d.grenade == 3 or range) then
...
function l:ffire_effect(ent)
	...
	if ed.strength <= 0 then
		if gun and gun.trig.missenemy and not ed.hitenemy then
			l.gunTrigger(gun, 'missenemy', .2)
		end
		ent:Remove()
	else
		if active then
			local hiten = false
			local fireflag = 0--DamageFlag.DAMAGE_FIRE
			local hitbox = 22 * ed.size
			for i, enemy in ipairs(Isaac.GetRoomEntities()) do
				if enemy:IsVulnerableEnemy() or enemy.Type == 292 then
					local diff = enemy.Position - ent.Position
					if math.abs(diff.X) < hitbox and math.abs(diff.Y) < hitbox then
						local type = enemy.Type
						hiten = true
						if gBAL.explosiveenemies[type] == -1 or gBAL.explosiveenemies[type] == enemy.Variant then
							enemy:TakeDamage(enemy.MaxHitPoints * 2, 0, EntityRef(ent), fireflag)
						else
							enemy:TakeDamage(ed.damage * freq, 0, EntityRef(ent), fireflag)
			...
			if hiten and not ed.hitenemy then
				ed.hitenemy = true
				if gun then
					if gun.trig.hitenemy then
						l.gunTrigger(gun, 'hitenemy', .2)
					end
					if gun.equipped then
						gun.juice = gun.juice + (ed.chargebonus or 1)
					end
				end
			end
			local room = game:GetRoom()
		...
		for i, en in ipairs(Isaac.GetRoomEntities()) do
			...
			elseif type == 5 and variant ~= 100 then
				...
				if dist < range and dist > 15 and math.abs(angleDiff(offset:GetAngleDegrees(), ang)) <= 90 then
					...
					if variant == 50 or variant == 52 or variant == 54 then
		...
		for i = 1, 4 do
			...
			if gun.copies[i] > 0 then
				for j = 1, gun.copies[i] do
					...
					wd.gun = gun
					wd.scale = gun.myuser.Type == 3 and gCON.famgunscale or 1
...
function l.gunMove(en, gun)
	...
	if gun and not game:IsPaused() then
		for h, fam in ipairs(d.gunners) do
			...
			local wepscale = (fam.Type == 3 and gCON.famgunscale) or 1
			local weplen = gWEP.BaseLength[gun.base] * wepscale
			local distbonus = ((gun.gunorbit and 20) or (gun.epic and 80) or 0)

			local disttgt = distbonus + mysize + 6 + weplen + 0--(gun.holddist or 0)
			...
			if gc.aiming and not (gun.rev and gun.rev < gun.revtime) then
				...
				--local shakedif = math.max(0, gun.spreadmax - gun.spreadmin)
				--local shake = shakedif > 0 and (Lerp(0, gun.spreadmin / 3, (gun.spread - gun.spreadmin) / shakedif or 0)) or 0
				--shakeoff = shake * Vector(0, 1):Rotated(en.FrameCount * 90).Y * (gun.shakemod or 1)
			end
			...
			if gun.reloading then
				disttgt = mysize + 2 + (weplen * .5)
				rotanimtgt = 65
				animspeed = .7
			elseif (idle or d.idle) and not fd.isdrone then
				idle = true
				disttgt = mysize + 10 + (weplen * .3) + (gun.epic and 15 or 0)
				rotanimtgt = -80
				aim = anim.side
				heighttgt = (gCON.gunheight * myheight * .75) - (weplen / 2)
				animspeed = .6
			else
				gunready = true
			...
			if weapons then
				for i = 1, 4 do if weapons[i] then
					...
					for j, wep in ipairs(set) do
						...
						wd.tearspawn = wep.Position + wep.Velocity + Vector(weplen - wd.recoil, 0):Rotated(myrot)
						...
						if gun.shotfx and not gun.noshotfx then
							...
							if spawnflash and not gun.sp[ESP.flamethrower] then
								...
								flash.Position = wep.Position + wep.Velocity + (Vector((weplen - wd.recoil) - 4, 0):Rotated(myrot + (anim.rot * invert)))
							end
						end
						...
						if bayonet then
							...
							bayonet.Position = wep.Position + Vector((weplen) + 10, 0):Rotated(wep.SpriteRotation * invert)
						...
						if gun.lasersight or gSET.AlwaysLaserSight then
							...
							laser.Position = wep.Position + Vector((weplen - (gun.nikita and 25 or 0)), 0):Rotated(wep.SpriteRotation * invert)
...
function l:muzzleFlash(effect)
	...
	if light and d.level then
		...
		if level <= .005 then
			...
			light.Color = Color(fclr.R, fclr.G, fclr.B, d.level * (gSET.ShotFlashBrightness / 25), 0, 0, 0)
		end
	else
...
function l:weaponUpdate(wep)
	...
	wep.SpriteScale = Vector(d.scale or 1, d.scale or 1)
		...
		for h, fam in ipairs(d.gunners) do
			...
			if weapons then
				for i = 1, 4 do
					...
					for j, wep in ipairs(set) do
						...
						if not gun.ksvictims then
							fam.Velocity = fam.Velocity + (recoil * ((fam.Type == 3) and 2.5 or 1))
						end
						...
						for k = 1, pelletnum do
							...
							if gun.fanshot then
								if gun.oarshot then
									if k > 1 and k < pelletnum then
										spreadrot = ((rng:RandomFloat() * usespread) - (.5 * usespread)) / 2.5
										spreadvel = 1 + (rng:RandomFloat() * .22)
										spreadscale = .9 + (rng:RandomFloat() * .3)
									else
							...
							--if (not gun.alternatinggunfire) or (gun.fireevents % (gun.alternatinggunfire or 1) == (j % (gun.alternatinggunfire or 1))) then
								...
								local laser = d.mygun.laser and (d.mygun.laser == 1 or (d.mygun.laser == 2 and gun.weaponsshot % 2 == 0) or (d.mygun.laserp5 and rng:RandomFloat() < .5))
								local bullet = false
								...
								if en.Type == 1 or en.Type == 3 then
									local vel = ((wd.direction * inherit) + (wd.direction * gun.velocity * 10 * spreadvel)):Rotated(spreadrot)
									if grenade then
									...
									elseif flame then
										...
										sd.faderate = 1/15
									elseif laser then
										if en.Type == 1 or fam:GetData().ingunners then
											shot = shooter:FireTechLaser(wd.tearspawn, 0, vel, true, true)
										elseif en.Type == 3 then
											shot = player:FireTechLaser(wd.tearspawn, 0, vel, true, true)
											--shot = Isaac.Spawn(7, 2, 0, wd.tearspawn, Vector(0, 0), shooter):ToLaser()
											shot.CollisionDamage = stat.Damage
											shot.TearFlags = 0
										end
										sd = shot:GetData()
										...
										if gun.useknifestatus and gun.myuser and gun.myuser:GetData().myknife then
											local kstatus = gun.myuser:GetData().myknife.status
											if kstatus == 'fire' then
												shot.TearFlags = setbit(shot.TearFlags, TearFlags.TEAR_BURN)
												shot:ChangeVariant(TearVariant.FIRE_MIND)
											elseif kstatus == 'poison' then
												shot.TearFlags = setbit(shot.TearFlags, TearFlags.TEAR_POISON)
												shot:GetSprite().Color = Color(.3, 1, .3, 1, 0, 0, 0)
											elseif kstatus == 'slow' then
												shot.TearFlags = setbit(shot.TearFlags, TearFlags.TEAR_SLOW)
												shot:GetSprite().Color = Color(.6, .6, .5, 1, 0, 0, 0)
											end
										end
								...
								if bullet then
									...
									shot.Scale = shot.Scale * gun.sizemult * spreadscale
				...
				if not gun.noshotfx then
					...
					flash:GetData().level = (gSET.ShotFlashBrightness / 100)
	...
	if gVAR.gunmode then
		...
		if pocket[card] then
			...
			if card == Card.CARD_CLUBS_2 then
				gVAR.grenadedrop = gVAR.grenadedrop + (gBAL.cards.grenadedropfor2clubs - (gBAL.grenadeperbomb * 2))
				local drop = l.getDropTable()
...
function l:take_dmg(ent, damage, flags, ref, cooldown)
	if gVAR.gunmode then
		...
		elseif ent:IsVulnerableEnemy() and damage > 0 and not hasbit(flags, DamageFlag.DAMAGE_FAKE) then
			...
			local player = Isaac.GetPlayer(0)
			--MINICRIT SHIT
			local fromfam = rtype == 3-- or (ref.Entity.SpawnerType == 3)
			local ed = ent:GetData()
			...
			if fromfam then
				...
				elseif famcrit[rvariant] then
					l.markForDeath(ent, {id = rvariant, add = famcrit[rvariant][1], mult = famcrit[rvariant][2]})
					return false
				end
			--valid for minicrit
			else
				...
				if rtype == 1 and hasbit(flags, DamageFlag.DAMAGE_LASER) then
					if gun and gun.critconfuse and (gun.laser or gun.laserp5) and (gun.crit or gun.wascrit) then
				...
				if deathmarks then
					...
					ed.deathmarks = nil
					sfx:Play(snd.misc.crit, .9, 0, false, .9 + (rng:RandomFloat() * .2))
				end
			end
			if ent:IsBoss() then
				local type = ent.Type
...
function l.getDropTable()
	...
	local drop = { --dropshit
		...
		bosssteal = {
			{2 * herbmult * gVAR.bossdropherbmult, 'bossherb', 'bherb'},
		...
		pickammo = {},
		pickherb = {
			{50, {5, 300, pocket[gPOC.herblist[1] ].id}}, -- green
			{1 * yellowmult * gVAR.bossdropyellowmult, {5, 300, pocket[gPOC.herblist[2] ].id}, 'yel'}, -- yellow (increases  yellow count when picked)
			{10 * rhmult, {5, 300, pocket[gPOC.herblist[3] ].id}}, -- red
		},
		bossherb = {
			{12, {5, 300, pocket[gPOC.herblist[1] ].id}}, -- green
			{1 * yellowmult, {5, 300, pocket[gPOC.herblist[2] ].id}, 'yel'}, -- yellow (increases  yellow count when picked)
			{1 * rhmult, {5, 300, pocket[gPOC.herblist[3] ].id}}, -- red
		},
		pickcoin = {
			{1, {5, 20, 0}}, -- random
			{9, {5, 20, 1}}, -- penny
		...
		pickgrenade = {
			...
			{10, {5, 300, pocket[gPOC.grenadelist[2] ].id}}, -- green
...
function l:tear_collision(tear, ent, low)
	if gVAR.gunmode then
		...
		if sd.isshot then
			...
			if sd.crit and (gBAL.explosiveenemies[ent.Type] == -1 or gBAL.explosiveenemies[ent.Type] == ent.Variant) then
				ent:TakeDamage(ent.MaxHitPoints * 2, 0, EntityRef(tear), DamageFlag.DAMAGE_EXPLOSION)
...
function l:entity_remove(ent)
	if gVAR.gunmode then
		...
		if type == 2 then
			...
			local gun = ed.gun
			if ed.crit then
				grid = game:GetRoom():GetGridEntityFromPos(ent.Position - ent.Velocity)
				if grid then
	...
	if gVAR.gunmode then
		...
		if room:IsFirstVisit() then
			...
			elseif roomtype == RoomType.ROOM_BOSS or roomtype == RoomType.ROOM_MINIBOSS then
				...
				log('boss id '..room:GetBossID())
				--todo: make this more reliable by checking specifically for boss id type against champ
		...
		if gVAR.bossdrops and npc:IsBoss() and (roomtype == RoomType.ROOM_BOSS or roomtype == RoomType.ROOM_MINIBOSS or roomtype == RoomType.ROOM_BOSSRUSH) then
			...
			if bosscount <= 1 then
				for i, reward in ipairs(gVAR.bossdrops) do
					...
					local reward = Isaac.Spawn(reward[1], reward[2], reward[3], room:FindFreePickupSpawnPosition(spos, 0, false), Vector(5 * (1 + (rng:RandomFloat() * .3)), 0):Rotated((360 / #gVAR.bossdrops) + math.random(30) + angle), nil)
				...
				for i, reward in ipairs(gVAR.bosscoins) do
					...
					local reward = Isaac.Spawn(reward[1], reward[2], reward[3], room:FindFreePickupSpawnPosition(spos, 0, false), Vector(8 * (1 + (rng:RandomFloat() * .3)), 0):Rotated((360 / #gVAR.bosscoins) + math.random(30) + angle), nil)
					--reward.GridCollisionClass = 6
				end
				gVAR.bossdrops = false
				gVAR.bosscoins = false
...
function l.gunHud(gamelayer)
	...
	--	local hudopacity = ((gSET.HudOpacity - 1) * .25) or .5
		local hudopacity = (gSET.HudOpacity / 100) or .5
...
function l.gunMenu(reload)
	local player = Isaac.GetPlayer(0)
	local d = player:GetData()
	local input = (reload and {}) or d.input.menu
	if not (screensize and d.input.menu) then
		return false
	end
	local scenter = getScreenCenterPosition()
	local menuspr = {CSP.MenuShadow, CSP.MenuBack, CSP.MenuFace, CSP.MenuBorder}
	local menuclr = gCON.menuclr
	local menupal = gSET.MenuPalette
	local rainbow = gSET.MenuPalette == 12
	local menumask = CSP.MenuMask
	local root1 = scenter + Vector(-42, 10)
	--local box1 = Vector(86, 60)
	local root2 = scenter + Vector(132, -10)
	local menuactive = false
	local money = player:GetNumCoins()
	--init
	...
	--background sprite
	if mmenuvar.open then
		if menuspr[3]:IsPlaying("Appear") or mmenuvar.openanim then
			mmenuvar.openanim = false
			for i = 1, 4 do
				menuspr[i]:Play("Appear")
			end
			mmenuvar.idle = false
		else
	...
	if not reload then
		--menuspr[1].Color = Color(1, 1, 1, 1, 0, 0, 0)
		--menuspr[4].Color = Color(1, 1, 1, 1, 0, 0, 0)
		menuspr[2].Color = menuclr[menupal][2]
		menuspr[3].Color = menuclr[menupal][1]
		for i = 1, 3 do
	...
	if menuactive then
		...
		if buttons and #buttons > 0 then
			...
			if button.variable or button.setting then
		...
		if pages and pages > 0 then
			...
			loadinv = loadinv or (item.inventory and psel ~= item.psel)
			psel = item.psel
			phov = item.phov
		...
		if loadinv then
			...
			local inv = mmenudir.inventory
			mmenuvar.inventory = {}
			local iitems = mmenuvar.inventory

			if merchant and mmenuvar.shop and gVAR.merchantinv then
			...
			for i = 1, 4 do
				if d.loadout[i] then
					table.insert(iitems, {kind = EIT.gun, id = d.loadout[i], ind = #iitems + 1})
				end
			end
			for i = 1, #d.mods do
				table.insert(iitems, {kind = EIT.mod, id = d.mods[i], ind = #iitems + 1})
			end
			local pocket0, pocket1 = player:GetCard(0), player:GetCard(1)
			if pocket0 ~= 0 and pocket[pocket0] then
				table.insert(iitems, {kind = EIT.pocket, id = pocket0, ind = #iitems + 1})
			end
			if pocket1 ~= 0 and pocket[pocket1] then
				table.insert(iitems, {kind = EIT.pocket, id = pocket1, ind = #iitems + 1})
			end

			local selid = iitems[item.psel or 1]
			inv.buttons = {}
			if not selid then
			elseif selid.merch then
				table.insert(inv.buttons, {str = 'buy', action = 'buy', price = getPrice(selid, 'buy')})
			else
				table.insert(inv.buttons, {str = 'sell', action = 'sell', price = getPrice(selid, 'sell')})
				if selid.kind == EIT.gun then
					table.insert(inv.buttons, {str = 'dismantle', action = 'destroy', price = getPrice(selid, 'destroy')})
					table.insert(inv.buttons, {str = 'customize'})
				elseif selid.kind == EIT.mod then
					table.insert(inv.buttons, {str = 'attach', action = 'pickmod'})
				end
			end
		...
		if draw then
			...
			--local boxoff = Vector(0, 8)
			--local box1 = Vector(-86, -54)
			--local box2 = Vector(86, 80)
			--local box1 = Vector(-86, -62)
			--local box2 = Vector(86, 79)
			--table.insert(drawings, {type = 'dot', root = root1, pos = Vector(0, 0)})
			--table.insert(drawings, {type = 'dot', root = root1, pos = box1})
			--table.insert(drawings, {type = 'dot', root = root1, pos = box2})

			if item.inventory and mmenuvar.inventory then
				local iitems = mmenuvar.inventory
				local iisel = iitems[item.psel]
				local id = iisel.id
				...
				for i = 1, disp do
					local mid = (disp + 1) / 2
					local ind = (((item.psel - 1) + (i - mid)) % #iitems) + 1
					local iitem = iitems[ind]
					local frame = (iitem.kind == EIT.gun and 13) or (iitem.kind == EIT.mod and 15) or 19
					local wid = (disp + 1) * 12
					local xoff = (wid / -2) + (i * 12)
					local alpha = (i == mid and 1) or (.65)
					local color = iitem.merch and menuclr[menupal][3] or menuclr[menupal][2]
					table.insert(drawings, {type = 'sym', frame = frame, color = color, alpha = alpha, root = root1, pos = Vector(xoff, -50)})
				end

				if iisel.kind == EIT.gun then
					local gunname = string.lower(id.name)--..(#id.mods > 0 and (' +'..#id.mods) or '')
					local gundesc = 'level '..#id.mods..' '..id.type
					table.insert(drawings,{type = 'str', str = gunname, size = 2, color = menuclr[menupal][3], root = root1, pos = Vector(0, -8), halign = 0})
					table.insert(drawings,{type = 'str', str = gundesc, size = 1, color = menuclr[menupal][2], root = root1, pos = Vector(0, 2), halign = 0})

					local wepui = CSP.Weapon
					wepui:ReplaceSpritesheet(0, gWEP.BaseSprite[iisel.id.base])
					wepui:LoadGraphics()
					wepui:Play("Idle")
					table.insert(drawings, {type = 'spr', sprite = wepui, shadow = true, color = CL_white, root = root1, pos = Vector(id.noammo and 0 or -16, -30 + minwob)})
					...
					if not id.noammo then
						...
						pickupui:Play("Idle")
						table.insert(drawings, {type = 'spr', sprite = CSP.Pickup, shadow = true, color = CL_white, root = root1, pos = Vector((gWEP.BaseLength[id.base]/2) + 4, -30 + 8 + minwob)})
					end
				elseif iisel.kind == EIT.mod then
					local modname = (gITM.moddat[id] and gITM.moddat[id].rename) or string.lower(config:GetCollectible(id).Name)
					local moddesc = string.lower(config:GetCollectible(id).Description)
					table.insert(drawings,{type = 'str', str = modname, size = 2, color = menuclr[menupal][3], root = root1, pos = Vector(0, -8), halign = 0})
					table.insert(drawings,{type = 'str', str = moddesc, size = 1, color = menuclr[menupal][2], root = root1, pos = Vector(0, 2), halign = 0})

					local modui = CSP.Mod
					modui:ReplaceSpritesheet(1, config:GetCollectible(id).GfxFileName);	modui:LoadGraphics()
					modui:Play("ShopIdle")
					table.insert(drawings, {type = 'spr', sprite = modui, shadow = true, color = CL_white, root = root1, pos = Vector(0, -20 + minwob)})
				elseif iisel.kind == EIT.pocket then -- pickup
					local pickupname = string.lower(config:GetCard(id).Name)
					local pickupdesc = string.lower(config:GetCard(id).Description)
					table.insert(drawings,{type = 'str', str = pickupname, size = 2, color = menuclr[menupal][3], root = root1, pos = Vector(0, -8), halign = 0})
					table.insert(drawings,{type = 'str', str = pickupdesc, size = 1, color = menuclr[menupal][2], root = root1, pos = Vector(0, 2), halign = 0})

					local pickupui = CSP.Pickup
					pickupui:ReplaceSpritesheet(0, pocket[id].sprite); pickupui:LoadGraphics()
					pickupui:Play("ShopIdle")
					table.insert(drawings, {type = 'spr', sprite = pickupui, shadow = true, color = CL_white, root = root1, pos = Vector(0, -24 + minwob)})
				end
			end

			--buttons
			if item.buttons then
				local boxbounds = {-86, -62, 86, 79}
				local width = 82
				local strheight = 0
				local seloff = 0
				local strset = {type = 'strset', set = {}, index = item, valign = valign, halign = halign, width = width, root = root1, pos = Vector(0, 0)}
				strset.bounds = boxbounds

				for i, btn in ipairs(item.buttons) do
					local clr1 = menuclr[menupal][2]
					local clr2 = menuclr[menupal][3]

					local bdraw = {type = 'str', index = btn, size = fsize, height = spacer, color = clr1, alpha = 1, select = bsel == i}
					bdraw.rainbow = rainbow or nil
					strheight = strheight + spacer
					table.insert(strset.set, bdraw)

					if btn.price then
						local bprice = -btn.price
						local vdraw = {type = 'str', halign = 1, size = fsize, height = 0, color = clr1, alpha = 1}
						vdraw.str = ((bprice > 0 and '+') or '')..tostring(bprice)..'_C'
						vdraw.color = ((bprice >= 0 and clr1) or clr2)
						vdraw.alpha = (money >= btn.price and 1) or .5
						table.insert(strset.set, vdraw)
					end

					if btn.variable or btn.setting then
						bdraw.height = bdraw.height + spacer
						local vdraw = {type = 'str', setind = btn, size = math.max(1, fsize - 1), height = 0, color = clr2, alpha = .8, pos = Vector(0, 14)}
						if btn.choices then
							vdraw.str = btn.choices[btn.setting]
						elseif btn.max then
							vdraw.str = (btn.pref or '')..btn.setting..(btn.suf or '')
						end
						vdraw.rainbow = rainbow or nil
			 	...
			 	if strheight > 16 * 7 then
					local halfheight = strheight / 2
					seloff = halfheight - (seloff - 16)
					seloff = math.max((halfheight - 64) * -1, math.min(halfheight - 64, seloff))
					...
					strset.pos = Vector(0, item.vscroll)
				end
				strset.pos = strset.pos + Vector(0, 64 * valign)
			...
			if item.title then
				...
				local title = {type = 'str', index = item.title, size = 3, color = menuclr[menupal][3], root = root1, pos = Vector(0, -74), halign = 0, underline = true}
	...
	if mmenuvar.idle and mmenuvar.maskalpha > 0 then
		local useclr = menuclr[menupal][1]
		menumask.Color = Color(useclr.R, useclr.G, useclr.B, mmenuvar.maskalpha, 0, 0, 0)
		--menumask.Color.AO = mmenuvar.maskalpha
		--menumask.Color = Color(1, 1, 1, mmenuvar.maskalpha, 0, 0, 0)
		menumask:Play("Idle", true)
		menumask:Render(root1, Vector(0, 0), Vector(0, 0))
	--	menumask:RenderLayer(1, root1)
	--	menumask:RenderLayer(2, root1)
	end
end

function l.drawMenu(tab)
	...
	local menuspr = CSP.MenuFace
	local menuclr = gCON.menuclr
	local menupal = gSET.MenuPalette
	local update = tab.update or false
	local alpha = tab.alpha or 1
	local color = tab.color or menuclr[menupal][2]
	if tab.rainbow then
		color = hsvToRgb((pos.Y % 256) / 255, 1, 1, 1)
	end

	color = Color(color.R, color.G, color.B, alpha, 0, 0, 0)
	...
	elseif dtype == 'spr' then
		...
		uspr.Scale = scale
		if tab.shadow then
			uspr.Color = Color(0, 0, 0, .5, 0, 0, 0)
			uspr:Render(root + pos + Vector(2, 2), Vector(0, 0), Vector(0, 0))
		end

		uspr.Color = color
		uspr:Render(root + pos, Vector(0, 0), Vector(0, 0))
	elseif dtype == 'str' then
		local ind = tab.index or {}
		if true or update or not ind.init then
			ind.size = tab.size or 1
			ind.str = tab.str or ind.str or 'nostring'
			ind.halign = tab.halign or 0
			ind.valign = tab.valign or 0
			ind.chr = {}
			local str = ind.str
			local idx = ind.size + 1
			local xoff = 0
			for i = 1, string.len(str) do
				local sub = string.sub(str, i, i)
				local len = mfdat[sub][idx]
				table.insert(ind.chr, {sub, xoff})
				xoff = xoff + len + 1
			end
			ind.len = xoff
			ind.init = true
		...
		--drawing string
		local fname = fnames[ind.size]
		local myscale = scaler[ind.size]
		font.Color = color
		font.Scale = scale
		local xoff = ((ind.halign == 0 and ind.len / -2) or (ind.halign == 1 and ind.len * -1) or 0) + ((tab.width or 82) * ind.halign)
		local yoff = ((ind.valign == 0 and -.5) or (ind.valign == 1 and -1) or 0) * (myscale)
		local clipt = 0
		local clipb = 0
		local usepos = pos + Vector(xoff, yoff)

		local wtf = ind.size == 2 and -3 or 0
		if tab.bounds then
		...
		for i, chr in ipairs(ind.chr) do
		...
		if tab.underline then
			...
			menuspr.Color = color
			menuspr:RenderLayer(3, root + pos + Vector(0, 16))
		end
		--select
		if tab.select then
			menuspr:SetFrame("Sym", 1)
			menuspr.Color = color
			local sympos = pos + Vector(xoff - 12, 3 - ind.size)
			--menuspr:RenderLayer(3, sympos)
			clipt = math.min(math.max(0, tab.bounds[2] - (sympos.Y - 8)), 16)
			clipb = math.min(math.max(0, (sympos.Y - 8) - tab.bounds[4]), 16)
			menuspr:Render(root + sympos, Vector(0, clipt), Vector(0, clipb))
		end
		--settings
		local setind = tab.setind
		if setind then
			local xplace = math.max(40, -xoff + 10)
			menuspr.Color = color
			if (setind.choices and setind.setting > 1) or (setind.max and setind.setting > (setind.min or 0)) then
				menuspr:SetFrame("Sym", 8)
				--menuspr:RenderLayer(3, root + pos + Vector(-60, -1))

				local sympos = pos + Vector(-xplace, -1)
				clipt = math.min(math.max(0, tab.bounds[2] - (sympos.Y - 8)), 16)
				clipb = math.min(math.max(0, (sympos.Y - 8) - tab.bounds[4]), 16)
				menuspr:Render(root + sympos, Vector(0, clipt), Vector(0, clipb))
			end
			if (setind.choices and setind.setting < #setind.choices) or (setind.max and setind.setting < setind.max) then
	...
	elseif dtype == 'strset' then
		local yy = 0
		for i, str in ipairs(tab.set) do
			str.root = root
			str.bounds = tab.bounds
			yy = yy + (str.height or 16)
			str.pos = (str.pos or Vector(0, 0)) + pos + Vector(0, yy)
			str.halign = str.halign or tab.halign
			str.width = str.width or tab.width or 82
		end
		local yo = (((tab.valign or -1) + 1) * yy) / -2

		for i, str in ipairs(tab.set) do
			str.pos = str.pos + Vector(0, yo)
			l.drawMenu(str)
		end
	end
end
